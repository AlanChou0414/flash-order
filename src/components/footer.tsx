...
const ShoppingCart = ({ show = true, item, isOpenConfirmPopup }: {
  show?: boolean;
  item?: ORDER_RECORD[];
  ...
  return (
    show &&
    <Box display="flex" justifyContent={item?.length ? "space-between" : ""} alignItems="center"
      zIndex={isOpenConfirmPopup ? 1400 : 1401} width="100%" bg="#353535" position="fixed" bottom={0} h="50px" >
      <Button borderRadius="0" h="100%" bg="#252525" position="relative" mr=".5rem"
      ...
      {item?.length
        ? <>
          ...
          <Button borderRadius="0" h="100%" bg="teal.500" color="white"
          ...
          <DrawerBody maxH={width > height ? "14rem" : "24rem"} minH={width > height ? "14rem" : "24rem"}>
            {item?.map((item: ORDER_RECORD) => (
              <Box key={item.id} display="flex" justifyContent="space-between" borderBottom=".5px solid #e3e3e3"
                py=".5rem">
                <HStack spacing={5}>
                  <Image src={`https://picsum.photos/150/150?random=${item.id}`} w="4rem" h="4rem" />
